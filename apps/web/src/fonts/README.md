...
[`../app/fonts.ts`](../app/fonts.ts) and
[`../app/manifesto/fonts.ts`](../app/manifesto/fonts.ts).
...
| `Newsreader-Variable.woff2` | Newsreader, wght 200–800 | SIL OFL 1.1 | `/manifesto` body copy only |
| `Newsreader-VariableItalic.woff2` | Newsreader Italic | SIL OFL 1.1 | `/manifesto` emphasis |
...
- **Axes nobody sets are instanced out.** Commit Mono ships an `ital` axis this
  site never uses. Newsreader ships `opsz` 6–72 and is pinned to 18, the
  optical size for body text — that alone takes it from 129K to 53K.
...
The whole site-wide stack is 92K, against 291K for the three faces it replaced.
